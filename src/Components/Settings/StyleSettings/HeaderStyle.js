...
const HeaderStyle = ({ attributes, setAttributes }) => {
  ...
  const [iconSpaceDevice, setIconSpaceDevice] = useState('desktop');
  ...
  return (
    <PanelBody title={__('Header Style', 'b-blocks')} initialOpen={false}>
      ...
      <div style={{ position: 'relative' }}>
        <Device device={paddingDevice} onChange={(value) => setPaddingDevice(value)} style={{ position: "absolute", top: "0", left: "55px" }} />
        <BoxControl
          label={__('Padding', 'data-block')}
          values={headerStyle.padding[paddingDevice]}
        ...
        <>
          <div style={{ position: 'relative' }}>
            <Device
              ...
              style={{ position: 'absolute', left: '115px', top: '0' }}
            />
            <RangeControl
              label={__('Icon/Image Size')}
              min={0}
              max={200}
              value={headerStyle.iconSize[iconSizeDevice]}
              onChange={(value) => update('iconSize', value, [iconSizeDevice])}
            />
          </div>
          <div style={{ position: 'relative' }}>
            <Device
              onChange={(value) => setIconSpaceDevice(value)}
              device={iconSpaceDevice}
              style={{ position: 'absolute', left: '115px', top: '0' }}
            />
            <RangeControl
              ...
              min={0}
              max={200}
              value={headerStyle.iconSpace[iconSpaceDevice]}
              onChange={(value) =>
                update('iconSpace', value, [iconSpaceDevice])
              }
            />
          </div>
          <PanelColorPicker
              ...
              >
                {icons &&
                  icons.map((icon, i) => (
                    <div
                      key={i}
                      onClick={() => update('titleAlign', icon.label.toLowerCase())}
                      className={`single-icon-admin-panel panelAlign ${headerStyle.titleAlign === icon.label.toLowerCase() ? 'isActive' : ''
                        }`}
                    >
                      {icon.value}
